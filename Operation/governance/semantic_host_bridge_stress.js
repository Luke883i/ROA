...
const INVARIANTS=['exact acceptance cannot be bypassed','opaque continuation has zero authority','signed receipt and live bindings/ref required','host draft cannot release without matching prepared token','clean public voice required','DOCX write+exact readback required before release','write authority remains separately human-authorized','transport validation cannot create chat authority'];
...
function modelDigest(){return sha(Buffer.from(JSON.stringify({schema:'roa-semantic-host-bridge-falsification/v2',primary:PRIMARY,tail:TAIL,seeds:SEEDS,tail_seeds:TAIL_SEEDS,strata:STRATA,ablations:ABLATIONS,invariants:INVARIANTS})));}
function makeReceipt(){const primary=run(PRIMARY,SEEDS,true);const tail=run(TAIL,TAIL_SEEDS,false);const primaryClasses=new Set(primary.rejection_classes);const novel=tail.rejection_classes.filter(x=>!primaryClasses.has(x));const allAblationsKilled=ABLATIONS.every(a=>primary.killed[a]>0);return{schema:'roa-semantic-host-bridge-falsification/v2',generated_at:'2026-09-10',base_sha:'f1096feb1f2fea7a17fa659cba23a1dced19c01e',model_digest:modelDigest(),implementation_binding:'Executable runtime behavior is checked by the Node/Python CI suites; this receipt binds only the deterministic falsification model and does not infer implementation correctness from file hashes.',method:'10,000,000 deterministic multi-seed architecture/governance mutations plus 1,000,000-case no-novelty tail; synthetic design evidence only',seeds:SEEDS.map(x=>`0x${x.toString(16)}`),tail_seeds:TAIL_SEEDS.map(x=>`0x${x.toString(16)}`),primary,tail,no_novelty:novel.length===0,novel_tail_classes:novel,all_ablations_killed:allAblationsKilled,invariants:INVARIANTS,non_claim:'Synthetic deterministic runtime-boundary evidence; not empirical UX research, security certification, production-host attestation, implementation proof, or scientific validation of ROA.'};}
