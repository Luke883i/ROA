...
PROMPT_SHA = "c0a9cc6bdb91ae1b1716a647f6338a21a9be34ddb6709171465669e948dfeea8"
REF = "f1096feb1f2fea7a17fa659cba23a1dced19c01e"
VERSION = "1.4.0"
...
class PromptBindingTests(unittest.TestCase):
    def test_prompt_body_hash_matches_contract(self):
        b = rt.bind_operating_prompt(expected_sha256=PROMPT_SHA, expected_version=PROMPT_VERSION)
        self.assertTrue(b.ok, b.reason)
        self.assertEqual(b.sha256, PROMPT_SHA)
        self.assertIn("IKANT UNIVERSAL META-PROMPT v3.1", b.body)

    def test_prompt_drift_fails_closed(self):
        with tempfile.TemporaryDirectory() as td:
            p = Path(td) / "prompt.md"
            p.write_text(rt.PROMPT.read_text(encoding="utf-8").replace("A1 representation!=reality", "A1 representation==reality", 1), encoding="utf-8")
            b = rt.bind_operating_prompt(expected_sha256=PROMPT_SHA, expected_version=PROMPT_VERSION, path=p)
            self.assertFalse(b.ok)
            self.assertIn(b.reason, {"prompt-self-digest-mismatch", "prompt-contract-digest-mismatch"})

    def test_prompt_marker_damage_fails_closed(self):
        with tempfile.TemporaryDirectory() as td:
            p = Path(td) / "prompt.md"
            p.write_text(f"IKANT_PROMPT_VERSION: {PROMPT_VERSION}\n", encoding="utf-8")
            self.assertFalse(rt.bind_operating_prompt(expected_sha256=PROMPT_SHA, expected_version=PROMPT_VERSION, path=p).ok)

class AccessContinuityTests(unittest.TestCase):
    def validate(self, r):
        return rt.validate_session_receipt(r, terms_sha256=TERMS, contract_version=VERSION, prompt_sha256=PROMPT_SHA, prompt_version=PROMPT_VERSION)

    def test_study_receipt_binds_acceptance_without_claiming_host_conformance(self):
        self.assertEqual(self.validate(receipt()).state, "STUDY_AUTHORIZED")

    def test_active_conforming_receipt_requires_prompt_readback(self):
        self.assertEqual(self.validate(receipt("ACTIVE_CONFORMING")).state, "ACTIVE_CONFORMING")

    def test_non_exact_acceptance_is_invalid(self):
        self.assertEqual(self.validate(receipt(accepted_command="I accept")).state, "RECEIPT_INVALID")

    def test_terms_drift_resets(self):
        self.assertEqual(self.validate(receipt(terms_sha256="x")).state, "RESET_REQUIRED")

    def test_contract_drift_resets(self):
        self.assertEqual(self.validate(receipt(contract_version="0")).state, "RESET_REQUIRED")

    def test_prompt_digest_drift_resets(self):
        self.assertEqual(self.validate(receipt(prompt_sha256="x")).state, "RESET_REQUIRED")

    def test_prompt_readback_drift_resets_only_conforming_state(self):
        self.assertEqual(self.validate(receipt("ACTIVE_CONFORMING", prompt_readback_sha256="x")).state, "RESET_REQUIRED")

    def test_study_state_cannot_claim_conformance(self):
        self.assertEqual(self.validate(receipt(conformance_status="CONFORMING")).state, "RECEIPT_INVALID")

    def test_degraded_read_only_is_retired(self):
        self.assertEqual(self.validate(receipt("DEGRADED_READ_ONLY")).state, "NOT_AUTHORIZED")
...
class SemanticPlaneTests(unittest.TestCase):
    ...
    def test_control_plane_encodes_current_hosted_chat_contract_without_authority(self):
        ...
        self.assertEqual(control["operating_prompt_version"], PROMPT_VERSION)
        ...
        self.assertEqual(control["continuity"], "opaque_application_handle_zero_authority")
        self.assertEqual(control["continuity_authority"], 0)
        self.assertEqual(control["artifact_release"], "write_plus_exact_readback_required")
    ...
    def test_pce_is_not_on_core_route(self):
        self.assertNotIn("PCE", json.loads(rt.RETICULUM.read_text(encoding="utf-8"))["core_route"])

    def test_witnesses_never_upgrade_authority(self):
        nodes={n["id"]:n for n in json.loads(rt.RETICULUM.read_text(encoding="utf-8"))["nodes"]}
...
class InferenceTests(unittest.TestCase):
    def test_hash_validity_cannot_fill_permission(self):
        self.assertIsNone(rt.claim_envelope(evidence_status="HashVerifiedOnly", debt=["validation"], terminal="Answer", receipt={"sha256":"x"})["permission"])

    def test_nonanswer_terminal_blocks_use(self):
        self.assertEqual(rt.named_use_gate(rt.claim_envelope(evidence_status="Unknown", debt=["source"], terminal="Unknown", receipt={}), adequate_horizon=True, blocking_debt=False, human_authority=True), "Blocked")

    def test_blocking_debt_requires_review(self):
        self.assertEqual(rt.named_use_gate(rt.claim_envelope(evidence_status="Supported", debt=["validation"], terminal="Answer", receipt={}), adequate_horizon=True, blocking_debt=True, human_authority=True), "ReviewRequired")

    def test_horizon_inadequate_precedes_permission(self):
        self.assertEqual(rt.named_use_gate(rt.claim_envelope(evidence_status="Supported", debt=[], terminal="Answer", receipt={}), adequate_horizon=False, blocking_debt=False, human_authority=True), "HorizonInadequate")
