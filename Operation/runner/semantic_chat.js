...
class SemanticChat {
  constructor(config = {}) {
    if (config.runtime) this.runtime = config.runtime;
    else {
      const { ChatRuntime } = require('./chat_runtime');
      this.runtime = new ChatRuntime(config);
    }
  }

  async askRepository(intent) {
    return this.project(await this.runtime.handleRepoRequest(intent));
  }

  async handleUserMessage(message) {
    return this.project(await this.runtime.handleUserMessage(message));
  }

  project(result) {
    if (!result || typeof result !== 'object') {
      return Object.freeze({ voice: SAFE_FAILURE_VOICE, artifact: null });
    }

    if (result.kind === 'PASS_THROUGH') {
      return Object.freeze({ voice: null, artifact: artifactFromInternalResult(result) });
    }

    const candidate = result.kind === 'ANSWER'
      ? result.voice
      : (CONTROL_VOICE[result.kind] || result.voice);
    const verdict = inspectSurfaceA(candidate);
    if (!verdict.ok) {
      if (this.runtime && typeof this.runtime.log === 'function') {
        this.runtime.log('PUBLIC_SURFACE_REJECTED', { issues: [...verdict.issues] });
      }
      return Object.freeze({
        voice: SAFE_FAILURE_VOICE,
